...
class WeldImpl(object):
    ...
    def div(self, array, other, type):
        ...
        """
        weld_obj = WeldObject(self.encoder_, self.decoder_)
    ...
    def aggr(self, array, op, initial_value, type):
        """
        Computes the aggregate of elements in the array.

        Args:
            array (WeldObject / encodable array): Input array to aggregate
    ...
    def dot(self, matrix, vector, matrix_type, vector_type):
        """
        Computes the dot product between a matrix and a vector.

        Args:
            matrix (WeldObject / encodable array): 2-d input matrix
            vector (WeldObject / encodable array): 1-d input vector
    ...
    def exp(self, array, type):
        """
        Computes the per-element exponenet of the passed-in array.

        Args:
            array (WeldObject / encodable array): Input array
            type (WeldType): Type of each element in the input array
...
numpy_weld_impl = WeldImpl(NumPyEncoder(), NumPyDecoder(), numpy_to_weld_type_mapping)