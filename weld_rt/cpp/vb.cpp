...
extern "C" void *new_vb(int64_t elem_size, int64_t starting_cap) {
  ...
  vb->thread_curs = new_merger(sizeof(vec_piece), get_nworkers());
...
extern "C" void new_piece(void *v, work_t *w) {
  ...
  vec_piece *cur_piece = (vec_piece *)get_merger_at_index(vb->thread_curs, sizeof(vec_piece),
    my_id_public());
...
extern "C" vec_piece *cur_piece(void *v, int32_t my_id) {
  vec_builder *vb = (vec_builder *)v;
  return (vec_piece *)get_merger_at_index(vb->thread_curs, sizeof(vec_piece), my_id);
}

extern "C" vec_output result_vb(void *v) {
  vec_builder *vb = (vec_builder *)v;
  for (int32_t i = 0; i < get_nworkers(); i++) {
    vec_piece *cur_piece = (vec_piece *)get_merger_at_index(vb->thread_curs, sizeof(vec_piece), i);
    if (cur_piece->data != NULL) {
      vb->pieces.push_back(*cur_piece);
    }
  }
