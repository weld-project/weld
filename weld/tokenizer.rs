...
fn parse_i16_literal(input: &str, base: u32) -> WeldResult<Token> {
    let slice = if base == 10 { &input[..input.len() - 2] } else { &input[2..input.len() - 2] };
...
fn basic_tokenize() {
    ...
    assert_eq!(tokenize("\"test string\"").unwrap(), vec![TStringLiteral("test string".to_string()), TEndOfInput]);
    assert_eq!(
        tokenize("\"test\" string").unwrap(),
        vec![TStringLiteral("test".to_string()), TIdent("string".into()), TEndOfInput]
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        vec![TBar, TIdent("a".into()), TColon, TI8, TBar, TExp, TOpenParen, TIdent("a".into()), TCloseParen, TEndOfInput]
    );
    assert_eq!(
        tokenize("|a:i8| log(a)").unwrap(),
        vec![TBar, TIdent("a".into()), TColon, TI8, TBar, TLog, TOpenParen, TIdent("a".into()), TCloseParen, TEndOfInput]
    );
    assert_eq!(
        tokenize("|a:i8| erf(a)").unwrap(),
        vec![TBar, TIdent("a".into()), TColon, TI8, TBar, TErf, TOpenParen, TIdent("a".into()), TCloseParen, TEndOfInput]
    );
    assert_eq!(
        tokenize("|a:i8| sqrt(a)").unwrap(),
        vec![TBar, TIdent("a".into()), TColon, TI8, TBar, TSqrt, TOpenParen, TIdent("a".into()), TCloseParen, TEndOfInput]
    );
    assert_eq!(tokenize("iffy if").unwrap(), vec![TIdent("iffy".into()), TIf, TEndOfInput]);

    assert_eq!(
        tokenize("keyexists(a, 1)").unwrap(),
        vec![TKeyExists, TOpenParen, TIdent("a".into()), TComma, TI32Literal(1), TCloseParen, TEndOfInput]
    );
