...
impl<'t> Parser<'t> {
    ...
    fn lambda_expr(&mut self) -> WeldResult<Box<PartialExpr>> {
        ...
        let body = try!(self.expr());
        Ok(expr_box(Lambda { params: params, body: body }, Annotations::new()))
    }
    ...
    fn comparison_expr(&mut self) -> WeldResult<Box<PartialExpr>> {
        ...
        // Unlike other expressions, we only allow one operator here; prevents stuff like a>b>c
        if *self.peek() == TLessThan || *self.peek() == TLessThanOrEqual || *self.peek() == TGreaterThan || *self.peek() == TGreaterThanOrEqual {
    ...
    fn apply_expr(&mut self) -> WeldResult<Box<PartialExpr>> {
        ...
        while *self.peek() == TDot || *self.peek() == TOpenParen {
            if *self.next() == TDot {
                    ...
                    TIdent(ref value) => {
                        if value.starts_with("$") {
                            match u32::from_str_radix(&value[1..], 10) {
                                Ok(index) => expr = expr_box(GetField { expr: expr, index: index }, Annotations::new()),
                ...
                try!(self.consume(TCloseParen));
                expr = expr_box(Apply { func: expr, params: params }, Annotations::new())
            }
        }
        Ok(expr)
    }
    ...
    fn parse_cast(&mut self, kind: ScalarKind) -> WeldResult<Box<PartialExpr>> {
        ...
        let cast_expr = expr_box(Cast { kind: kind, child_expr: expr }, Annotations::new());
        Ok(cast_expr)
    }
    ...
    fn unary_leaf_expr(&mut self, token: Token) -> WeldResult<Box<PartialExpr>> {
        ...
        Ok(expr_box(UnaryOp { kind: kind, value: value }, Annotations::new()))
    }
    ...
    fn leaf_expr(&mut self) -> WeldResult<Box<PartialExpr>> {
        ...
        match *self.next() {
            ...
            TLookup => {
                ...
                try!(self.consume(TCloseParen));
                Ok(expr_box(Lookup { data: data, index: index }, Annotations::new()))
            }
            ...
            TKeyExists => {
                ...
                try!(self.consume(TCloseParen));
                Ok(expr_box(KeyExists { data: data, key: key }, Annotations::new()))
            }
            ...
            TSort => {
                ...
                try!(self.consume(TCloseParen));
                Ok(expr_box(Sort { data: data, keyfunc: keyfunc }, Annotations::new()))
            }
            ...
            TMerge => {
                ...
                let value = try!(self.expr());
                try!(self.consume(TCloseParen));
                Ok(expr_box(Merge { builder: builder, value: value }, Annotations::new()))
            }
            ...
            TDictMerger => {
                ...
                expr.ty = Builder(
                    DictMerger(
                        ...
                        Box::new(Struct(vec![key_type.clone(), value_type.clone()])),
                        bin_op,
                    ),
                    annotations,
                );
            ...
            TVecMerger => {
                ...
                expr.ty = Builder(
                    VecMerger(Box::new(elem_type.clone()), Box::new(Struct(vec![Scalar(ScalarKind::I64), elem_type.clone()])), bin_op),
                    annotations,
                );
                Ok(expr)
            }
    ...
    fn commutative_binop_(&mut self) -> WeldResult<BinOpKind> {
        match *self.peek() {
            ...
            ref t => weld_err!("expected commutative binary op in but got '{}'", t),
        }
    }
    ...
    fn type_(&mut self) -> WeldResult<PartialType> {
        ...
        match *self.next() {
            ...
            TVecMerger => {
                ...
                Ok(Builder(
                    VecMerger(Box::new(elem_type.clone()), Box::new(Struct(vec![Scalar(ScalarKind::I64), elem_type.clone()])), binop),
                    annotations,
                ))
            }
...
fn basic_parsing() {
    ...
    assert_eq!(print_expr_without_indent(&e), "(let a=((3+2));(let b=((let c=(a);c));b))");
    ...
    assert_eq!(print_expr_without_indent(&e), "@(impl:local,num_keys:12)dictmerger[i32,i32,+]");
...
fn operator_precedence() {
    ...
    assert_eq!(print_expr_without_indent(&e), "(a||(b&&(c|(d^(e&(f==(g>(h+(i*j)))))))))");
