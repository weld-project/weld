...
impl WeldModule {
    ...
    pub fn compile<S: AsRef<str>>(code: S, conf: &WeldConf) -> WeldResult<WeldModule> {
        ...
        let _ = codegen::llvm2::compile_program(&program, parsed_conf, &mut stats)?;

        // Comment out this line to re-enable old runtime.
        return weld_err!("Finished compilation using llvm2 backend - blocking old compilation.");
