...
pub fn lambda_expr(params: Vec<Parameter<Type>>, body: Expr<Type>) -> WeldResult<Expr<Type>> {
    let ty = Function(params.iter().map(|p| p.ty.clone()).collect(), Box::new(body.ty.clone()));
    new_expr(
        Lambda {
            params: params,
            body: Box::new(body),
...
pub fn newbuilder_expr(kind: BuilderKind, expr: Option<Expr<Type>>) -> WeldResult<Expr<Type>> {
    ...
    new_expr(NewBuilder(expr.map(|e| Box::new(e))), Builder(kind, Annotations::new()))
}
...
fn builder_exprs_test() {
    ...
    assert_eq!(print_expr_without_indent(&for_loop), "for([5],merger[i32,+],|b,i,e|merge(b,e))");
