...
impl LlvmGenerator {
    ...
    fn gen_unload_arg_struct(
        ...
        ctx.code.add(format!(
            "{} = getelementptr inbounds %work_t, %work_t* %cur.work, i32 0, i32 0",
            work_data_ptr
        ));
        ctx.code
        ...
        ctx.code.add(format!(
            "{} = bitcast i8* {} to {}*",
            storage_ptr, work_data, arg_struct_ll_ty
        ));
    ...
    fn gen_create_new_vb_pieces_helper(
        &mut self,
        sym: &str,
        ty: &Type,
        inner_ctx: &mut FunctionContext,
        outer_ctx: &mut FunctionContext,
    ) -> WeldResult<(bool)> {
        ...
        match *ty {
            Builder(ref bk, _) => match *bk {
                Appender(_) => {
                    ...
                    inner_ctx.code.add(format!(
                        "call void {}.newPiece({} {}, %work_t* %cur.work)",
                        bld_prefix, bld_ty_str, sym
                    ));
                    has_builder = true;
                }
                _ => {}
            ...
            Struct(ref fields) => {
                for (i, f) in fields.iter().enumerate() {
                    ...
                    inner_ctx.code.add(format!(
                        "{} = extractvalue {} {}, {}",
                        bld_sym, struct_ty, sym, i
                    ));
                    has_builder |=
                        self.gen_create_new_vb_pieces_helper(&bld_sym, f, inner_ctx, outer_ctx)?;
                }
            }
            _ => {}
        }
    ...
    fn gen_create_new_vb_pieces(
        ...
        ctx.code.add(format!(
            "{} = getelementptr inbounds %work_t, %work_t* %cur.work, i32 0, i32 4",
            full_task_ptr
        ));
        ctx.code.add(format!(
            "{} = load i32, i32* {}",
            full_task_int, full_task_ptr
        ));
        ctx.code.add(format!(
            "{} = trunc i32 {} to i1",
            full_task_bit, full_task_int
        ));
        ctx.code.add(format!(
            "br i1 {}, label %new_pieces, label %fn_call",
            full_task_bit
        ));
        ...
        for (arg, ty) in params_sorted.iter() {
            ...
            let has_builder = self.gen_create_new_vb_pieces_helper(
                format!("{}{}", llvm_symbol(arg), suffix).as_str(),
                ty,
                inner_ctx,
                ctx,
            )?;
    ...
    fn gen_create_stack_mergers_helper(
        &mut self,
        sym: &str,
        ty: &Type,
        inner_ctx: &mut FunctionContext,
            ...
            Struct(ref fields) => {
                for (i, f) in fields.iter().enumerate() {
                    ...
                    inner_ctx.code.add(format!(
                        ...
                        bld_sym, struct_ty, struct_ty, sym, i
                    ));
                    has_builder |=
                        self.gen_create_stack_mergers_helper(&bld_sym, f, inner_ctx, outer_ctx)?;
                }
            }
            _ => {}
        }
            ...
            match *ty {
                Builder(ref bk, _) => match *bk {
                    Merger(ref val_ty, ref op) => {
                        ...
                        let stack_simd =
                            self.gen_load_var(&stack_ptr_simd, &val_ll_simd_ty, ctx)?;
                        let stack_scalar =
                            self.gen_load_var(&stack_ptr_scalar, &val_ll_scalar_ty, ctx)?;
                        self.gen_merge_op(
                            &bld_ptr_simd,
                            &stack_simd,
                            &val_ll_simd_ty,
            ...
            match *ty {
                Builder(ref bk, _) => match *bk {
                    Merger(ref val_ty, ref op) => {
                        ...
                        ctx.code.add(format!(
                            "{} = call i1 {}.isGlobal({}* {})",
                            is_global, bld_prefix, bld_ll_ty, bld_ll_sym
                        ));
                        ctx.code.add(format!(
                            "br i1 {}, label {}, label {}",
                            is_global, cur_label, next_label
                        ));
            ...
            match *ty {
                Builder(ref bk, _) => match *bk {
                    Merger(ref val_ty, ref op) => {
                        ...
                        let reg_scalar =
                            self.gen_load_var(&reg_ptr_scalar, &val_ll_scalar_ty, ctx)?;
                        self.gen_merge_op(
                            &bld_ptr_simd,
                            &reg_simd,
                            &val_ll_simd_ty,
    ...
    fn gen_create_arg_struct(
        ...
        let ll_ty = self
            .llvm_type(&Struct(params_sorted.values().map(|e| e.clone()).collect()))?
            .to_string();
        for (i, (arg, ty)) in params_sorted.iter().enumerate() {
    ...
    fn gen_num_iters_and_fringe_start(
        &mut self,
        par_for: &ParallelForData,
        func: &SirFunction,
        ctx: &mut FunctionContext,
    ) -> WeldResult<(String, Option<String>)> {
        ...
        match par_for.data[0].kind {
            ...
            IterKind::NdIter => {
                ...
                let (cur_i_ptr, cur_i) = self.add_llvm_for_loop_start(
                    ctx,
                    &loop_name,
                    "0",
                    &shape_llvm_info.len_str,
                    "slt",
                )?;
                ...
                ctx.code
                    .add(format!("{} = mul i64 {}, {}", tmp_result, prod, shape_i));
        ...
        if par_for.data.len() > 1 || par_for.data[0].start.is_some() {
            for (i, iter) in par_for.data.iter().enumerate() {
                if iter.kind == IterKind::NdIter {
                    ...
                    ctx.code
                        .add(format!("{} = load i64, i64* {}", final_offset, offset_ptr));
                    ctx.code.add(format!(
                        "{} = add i64 {}, {}",
                        max_iter_str, start_str, final_offset
                    ));
    ...
    fn gen_invoke_loop_body(
        &mut self,
        num_iters_str: &str,
        par_for: &ParallelForData,
        sir: &SirProgram,
        func: &SirFunction,
        mut ctx: &mut FunctionContext,
    ) -> WeldResult<()> {
        ...
        let cont_struct =
            self.gen_create_arg_struct(&sir.funcs[par_for.cont].params, ".ac", ctx)?;
        ctx.code.add(format!(
    ...
    fn add_llvm_for_loop_start(
        ...
        ctx.code.add(format!(
            "{} = icmp {} i64 {}, {}",
            cmp_str, cmp_type, cur_i, end_cmp
        ));
        ctx.code.add(format!(
            "br i1 {}, label %{name}.body, label %{name}.end",
            cmp_str,
            name = loop_name
        ));
    ...
    fn nditer_next_element(
        ...
        let (cur_i_ptr, cur_i) =
            self.add_llvm_for_loop_start(ctx, &loop_name, "0", &strides_llvm_info.len_str, "slt")?;
        ...
        ctx.code.add(format!(
            "{id} = getelementptr i64, i64* %counter.idx, i64 {idx}",
            id = counter_i_ptr,
            idx = cur_i
        ));
        ...
        ctx.code
            .add(format!("store i64 {}, i64* {}", tmp_sum2, sum_ptr));
    ...
    fn gen_loop_iteration_start(
        ...
        for (i, iter) in par_for.data.iter().enumerate() {
            ...
            let data_llvm_info =
                self.get_array_llvm_info(func, ctx, &iter.data, inner_elem_ty_str.clone(), true)?;
            ...
            let arr_idx = if iter.kind == IterKind::NdIter {
                ...
                if iter.kind == IterKind::FringeIter {
                    ...
                    ctx.code.add(format!(
                        "{} = call i64 {}.size({} {})",
                        arr_len,
                        data_llvm_info.prefix,
                        data_llvm_info.ty_str,
                        data_llvm_info.arr_str
                    ));
                    ctx.code
        ...
        ctx.code.add(format!(
            "store {} {}, {}* {}",
            &elem_ty_str, prev_ref, &elem_ty_str, elem_str
        ));
    ...
    fn gen_loop_iteration_end(
        ...
        if nditer.is_some() {
            ...
            let shape_elem_str =
                self.get_array_idx(ctx, shape_llvm_info, false, &"%cur_i".to_string())?;
            ctx.code.add(format!(
                "{tmp5} = load i64, i64* {i}
    ...
    fn gen_parallel_runtime_callback_function(&mut self, func: &SirFunction) -> WeldResult<()> {
        ...
        ctx.code.add(format!(
            "{} = getelementptr %work_t, %work_t* %cur.work, i32 0, i32 1",
            lower_bound_ptr
        ));
        ctx.code.add(format!(
            "{} = load i64, i64* {}",
        ...
        ctx.code.add(format!(
            "{} = getelementptr %work_t, %work_t* %cur.work, i32 0, i32 2",
            upper_bound_ptr
        ));
        ctx.code.add(format!(
            "{} = load i64, i64* {}",
            upper_bound, upper_bound_ptr
        ));
        ...
        ctx.code.add("}\n\n");
        self.body_code.add(format!(
            "define void @f{}_par(%work_t* %cur.work) {{",
            func.id
        ));
    ...
    fn gen_loop_continuation_function(
        &mut self,
        par_for: &ParallelForData,
        sir: &SirProgram,
    ) -> WeldResult<()> {
    ...
    fn llvm_type(&mut self, ty: &Type) -> WeldResult<String> {
        Ok(match *ty {
            ...
            _ => return compile_err!("Unsupported type {}", ty)?,
        })
    }
    ...
    fn gen_eq(&mut self, ty: &Type) -> WeldResult<()> {
        ...
        let eq_on_pointers = format!(
            include_str!("resources/eq_on_pointers.ll"),
            TYPE = &ll_ty,
        ...
        match *ty {
            Struct(ref fields) => {
                ...
                for i in 0..field_types.len() {
                    ...
                    self.prelude_code.add_line(format!(
                        "{} = call i32 {}.hash({} {})",
                        hash, field_prefix_str, field_ty_str, field
                    ));
                    self.prelude_code.add_line(format!(
                        "{} = call i32 @hash_combine(i32 {}, i32 {})",
                        new_res, res, hash
                    ));
                    res = new_res;
                }
    ...
    fn gen_merge_op_on_registers(
        ...
        match *arg_ty {
            Scalar(_) | Simd(_) => match *bin_op {
                Max | Min => {
                    assert!(self
                        .gen_minmax(&llvm_ty, bin_op, arg1, arg2, &res, arg_ty, var_ids, code)
                        .is_ok());
                }
                _ => {
                    code.add(format!(
            ...
            Struct(ref fields) => {
                ...
                for (i, ty) in fields.iter().enumerate() {
                    ...
                    code.add(format!(
                        "{} = extractvalue {} {}, {}",
                        &arg1_elem, &llvm_ty, &arg1, i
                    ));
                    code.add(format!(
                        "{} = extractvalue {} {}, {}",
                        &arg2_elem, &llvm_ty, &arg2, i
                    ));
            ...
            _ => {
                return compile_err!(
                    "gen_merge_op_on_registers called on invalid type {}",
                    arg_ty
                );
            }
        }
    ...
    fn gen_serialize_helper(
        ...
        match *expr_ty {
            ...
            Scalar(_) | Struct(_) if !expr_ty.has_pointer() => {
                ...
                buffer_ll_ty));
                serialize_code.add(format!(
                    "%tmp2 = call i8* @{}.growable.last({}.growable %tmp)",
            ...
            Vector(ref elem_ty) if elem_ty.has_pointer() => {
                ...
                let elem_serialize = self.gen_serialize_helper(
                    buffer_ll_ty,
                    buffer_ll_prefix,
                    elem_ll_ty,
            ...
            Dict(ref key, ref value) => {
                ...
                let _ = self.gen_serialize_helper(
                    buffer_ll_ty,
                    buffer_ll_prefix,
                    key_ll_ty,
                ...
                self.prelude_code.add(format!(
                    ...
                    BUFNAME = buffer_ll_ty.replace("%", ""),
                    HAS_POINTER = 1,
                    KEY_SERIALIZE_ON_PTR =
                        format!("{}.serialize_on_pointers", llvm_prefix(key_ll_ty)),
                    VAL_SERIALIZE_ON_PTR =
                        format!("{}.serialize_on_pointers", llvm_prefix(value_ll_ty))
                ));
            }
            Struct(ref tys) => {
    ...
    fn gen_deserialize_helper(
        ...
        match *output_ty {
            ...
            Struct(ref tys) => {
                ...
                deserialize_code.add(format!(
                    "define i64 {}({} %buf, i64 %offset, {}* %resPtr) {{",
                    deserialize_fn, buffer_ll_ty, output_ll_ty
                ));
    ...
    fn gen_deserialize(
        ...
        ctx.code
            .add("; Check to ensure that the full vector was consume during deserialization.");
        ctx.code.add(format!(
            "{} = call i64 {}.size({} {})",
            array_size, expr_ll_prefix, expr_ll_ty, expr_tmp
        ));
        ctx.code.add(format!(
            "{} = icmp ne i64 {}, {}",
            cond, bytes_tmp, array_size
        ));
        ctx.code.add(format!(
            "br i1 {}, label {}, label {}",
            cond, abort_label, continue_label
        ));
    ...
    fn gen_serialize(
        ...
        let serialize_fn = self.gen_serialize_helper(
            &output_ll_ty,
            &output_ll_prefix,
            &expr_ll_ty,
        ...
        ctx.code.add(format!(
            "{} = call {}.growable @{}.growable.new(i64 1024)",
        ...
        match statement.kind {
            ...
            MakeVector(ref elems) => {
                ...
                for (i, elem) in elems.iter().enumerate() {
                    ...
                    ctx.code.add(format!(
                        "{} = call {}* {}.at({} {}, i64 {})",
                        ptr, elem_ll_ty, output_ll_prefix, output_ll_ty, vec, i
                    ));
            ...
            BinOp {
                ...
                match *ty {
                    ...
                    Vector(_) => {
                        ...
                        ctx.code.add(format!(
                            "{} = call i32 {}.cmp({} {}, {} {})",
                            tmp, vec_prefix, ll_ty, left_tmp, ll_ty, right_tmp
                        ));
                        ctx.code.add(format!(
                            "{} = icmp {} i32 {}, {}",
                            output_tmp, op_name, tmp, value
                        ));
                        self.gen_store_var(&output_tmp, &output_ll_sym, &output_ll_ty, ctx);
                    }
                    ...
                    Vector(ref elem_ty) => {
                        ...
                        if param_syms.len() == 1 {
                            ...
                            if *param_ty == **elem_ty {
                                ...
                                return compile_err!(
                                    "Type mismatch: vector:{ } and sort key parameter:{ }",
                                    &**elem_ty,
                                    &*param_ty
                                );
                            }
                        ...
                        if let Terminator::ProgramReturn(ref key_sym) = last_block.terminator {
                            ...
                            match *key_ty {
                                ...
                                _ => {
                                    return compile_err!(
                                        "Sort key function must have scalar or vector return type"
                                    );
                                }
                            }
                        } else {
    ...
    fn gen_merge(
        ...
        match *builder_kind {
            ...
            VecMerger(ref t, ref op) => {
                ...
                ctx.code.add(format!(
                    "{} = bitcast i8* {} to {}*",
                    bld_ptr, bld_ptr_raw, elem_ll_ty
                ));
    ...
    fn gen_result(
        ...
        match *builder_kind {
            Appender(_) => {
                ...
                ctx.code.add(format!(
                    "{} = call {} {}.result({} {})",
                    res_tmp, res_ll_ty, bld_prefix, bld_ll_ty, bld_tmp
                ));
            ...
            DictMerger(_, _, _) | GroupMerger(_, _) => {
                ...
                ctx.code.add(format!(
                    "{} = call {} {}.result({} {})",
                    res_tmp, res_ty_str, bld_prefix, bld_ty_str, bld_tmp
                ));
        ...
        match *builder_kind {
            ...
            Merger(ref elem_ty, ref op) => {
                ...
                let init_elem = match *arg {
                    Some(ref s) => {
                        let arg_str =
                            self.gen_load_var(llvm_symbol(s).as_str(), &elem_type, ctx)?;
                        arg_str
                    }
                    _ => iden_elem.clone(),
                };
        ...
        match *terminator {
            ...
            ProgramReturn(ref sym) => {
                ...
                ctx.code.add(format!(
                    ...
                    &elem_size_ptr, &ty_str, &ty_str
                ));
                ctx.code.add(format!(
                    "{} = ptrtoint {}* {} to i64",
                    &elem_size, &ty_str, &elem_size_ptr
                ));

                ctx.code
                    .add(format!("{} = call i64 @weld_rt_get_run_id()", run_id));
                ctx.code.add(format!(
                    "{} = call i8* @weld_run_malloc(i64 {}, i64 {})",
                    &elem_storage, &run_id, &elem_size
                ));
                ctx.code.add(format!(
                    "{} = bitcast i8* {} to {}*",
                    &elem_storage_typed, &elem_storage, &ty_str
                ));
