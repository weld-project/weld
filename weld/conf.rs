...
pub fn parse(conf: &WeldConf) -> WeldResult<ParsedConf> {
    ...
    let memory_limit = value.map(|s| parse_memory_limit(&s)).unwrap_or(Ok(DEFAULT_MEMORY_LIMIT))?;

    let value = get_value(conf, THREADS_KEY);
    let threads = value.map(|s| parse_threads(&s)).unwrap_or(Ok(DEFAULT_THREADS))?;

    let value = get_value(conf, OPTIMIZATION_PASSES_KEY);
    let mut passes = value.map(|s| parse_passes(&s)).unwrap_or(Ok(DEFAULT_OPTIMIZATION_PASSES.clone()))?;
    ...
    let level = value.map(|s| parse_llvm_optimization_level(&s)).unwrap_or(Ok(DEFAULT_LLVM_OPTIMIZATION_LEVEL))?;
    ...
    let dump_code_dir = value.map(|s| parse_dir(&s)).unwrap_or(Ok(DEFAULT_DUMP_CODE_DIR.clone()))?;

    let value = get_value(conf, DUMP_CODE_KEY);
    let dump_code_enabled = value.map(|s| parse_bool_flag(&s, "Invalid flag for dumpCode")).unwrap_or(Ok(DEFAULT_DUMP_CODE))?;

    let value = get_value(conf, SIR_OPT_KEY);
    let sir_opt_enabled = value.map(|s| parse_bool_flag(&s, "Invalid flag for sirOptimization")).unwrap_or(Ok(DEFAULT_SIR_OPT))?;
