...
use super::ast::{Expr, ExprKind, Symbol, Type};
...
fn parse_and_print_uniquified_expressions() {
    ...
    let _ = uniquify(&mut e);
    assert_eq!(print_expr_without_indent(&e).as_str(), "(let a=(2);(let a__1=(3);a__1))");

    // Make sure Let values aren't renamed.
    let mut e = parse_expr("let a = 2; let a = a+1; a").unwrap();
    let _ = uniquify(&mut e);
    assert_eq!(print_expr_without_indent(&e).as_str(), "(let a=(2);(let a__1=((a+1));a__1))");

    // Lambdas and proper scoping.
    let mut e = parse_expr("let a = 2; (|a,b|a+b)(1,2) + a").unwrap();
    let _ = uniquify(&mut e);
    assert_eq!(print_expr_without_indent(&e).as_str(), "(let a=(2);((|a__1,b|(a__1+b))(1,2)+a))");
...
fn parse_and_print_typed_expr_without_indentations() {
    ...
    let mut e = parse_expr("let a = 2; a").unwrap();
    assert_eq!(print_typed_expr_without_indent(&e).as_str(), "(let a:?=(2);a:?)");
    infer_types(&mut e).unwrap();
    assert_eq!(print_typed_expr_without_indent(&e).as_str(), "(let a:i32=(2);a:i32)");

    let mut e = parse_expr("let a = 2; let a = false; a").unwrap();
    infer_types(&mut e).unwrap();
    assert_eq!(print_typed_expr_without_indent(&e).as_str(), "(let a:i32=(2);(let a:bool=(false);a:bool))");

    // Types should propagate from function parameters to body
    let mut e = parse_expr("|a:i32, b:i32| a + b").unwrap();
    infer_types(&mut e).unwrap();
    assert_eq!(print_typed_expr_without_indent(&e).as_str(), "|a:i32,b:i32|(a:i32+b:i32)");

    let mut e = parse_expr("|a:f32, b:f32| a + b").unwrap();
    infer_types(&mut e).unwrap();
    assert_eq!(print_typed_expr_without_indent(&e).as_str(), "|a:f32,b:f32|(a:f32+b:f32)");

    let mut e = parse_expr("let a = [1, 2, 3]; 1").unwrap();
    infer_types(&mut e).unwrap();
    assert_eq!(print_typed_expr_without_indent(&e).as_str(), "(let a:vec[i32]=([1,2,3]);1)");
