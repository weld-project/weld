...
pub trait GenCmp {
    ...
    /// well.
    unsafe fn gen_custom_cmp(
        &mut self,
        elem_ty: LLVMTypeRef,
        cf_id: FunctionId,
        cmpfunc: LLVMValueRef,
...
impl GenCmp for LlvmGenerator {
    ...
    unsafe fn gen_cmp_fn(&mut self, ty: &Type) -> WeldResult<LLVMValueRef> {
        ...
        match *ty {
            ...
            Struct(ref elems) => {
                // recursively apply cmp to elements
                ...
                for (i, elem) in elems.iter().enumerate() {
                    ...
                    let cond =
                        LLVMBuildICmp(builder, LLVMIntEQ, field_result, self.i32(0), c_str!(""));
            ...
            Vector(ref elem) if elem.supports_memcmp() => {
                ...
                let bytes_equal =
                    LLVMBuildICmp(builder, LLVMIntEQ, memcmp_result, self.i32(0), c_str!(""));
                let result =
                    LLVMBuildSelect(builder, bytes_equal, size_eq, memcmp_result, c_str!(""));
            ...
            Vector(ref elem) => {
                ...
                let bytes_equal =
                    LLVMBuildICmp(builder, LLVMIntEQ, result, self.i32(0), c_str!(""));
