...
fn process_loadfile(arg: &str) -> Result<String, String> {
    ...
    match File::open(&path) {
        Err(why) => {
            return Err(format!("Error: couldn't open {}: {}", path_display, why.description()));
        }
        Ok(res) => {
            file = res;
        }
    ...
    match file.read_to_string(&mut contents) {
        Err(why) => {
            return Err(format!("Error: couldn't read {}: {}", path_display, why.description()));
        }
        _ => {}
    }
...
fn process_code(code: &str, conf: *mut WeldConf) {
    unsafe {
        ...
        if weld_error_code(err) != WeldRuntimeErrno::Success {
            println!("REPL: Compile error: {}", CStr::from_ptr(weld_error_message(err)).to_str().unwrap());
        } else {
...
fn main() {
    ...
    let matches = App::new("Weld REPL")
        ...
        .arg(Arg::with_name("dumpcode").short("d").long("dumpcode").help("Dump code to file").takes_value(false))
        .arg(
            Arg::with_name("input")
                .short("i")
                .long("input")
