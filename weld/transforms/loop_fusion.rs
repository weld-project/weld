                    ...
                    let new_merge_expr = Expr {
                        ...
                        kind: Merge {
                            ...
                            value: Box::new(Expr {
                                ty: merge_type.clone(),
                                kind: MakeStruct {
                                    elems: lambdas.iter().map(|ref lambda| *lambda.1.clone()).collect::<Vec<_>>(),
                                },
                                annotations: Annotations::new(),
                            }),
                        },
...
fn consumes_all(iter: &Iter<Type>) -> bool {
    ...
    } = iter
    {
        ...
        if let (&Literal(I64Literal(1)), &Literal(I64Literal(0)), &Ident(ref name), &Length { data: ref v }) = (&stride.kind, &start.kind, &data.kind, &end.kind) {
            if let Ident(ref vsym) = v.kind {
                return vsym == name;
            }
        }
        // Checks if an entire vector literal is consumed.
        if let (&Literal(I64Literal(1)), &Literal(I64Literal(0)), &MakeVector { ref elems }) = (&stride.kind, &start.kind, &data.kind) {
...
fn replace_builder(lambda: &Expr<Type>, nested: &Expr<Type>, sym_gen: &mut SymbolGenerator) -> WeldResult<Expr<Type>> {
    ...
    } = lambda.kind
    {
        ...
        } = nested.kind
        {
            ...
            new_body.transform_and_continue_res(&mut |ref mut e| match e.kind {
                ...
                Ident(ref mut symbol) if *symbol == old_bldr.name => Ok((Some(new_bldr.clone()), false)),
                Ident(ref mut symbol) if *symbol == old_index.name => Ok((Some(new_index.clone()), false)),
                _ => Ok((None, true)),
            });
