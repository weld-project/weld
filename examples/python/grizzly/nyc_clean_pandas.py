...
def run(filename, use_grizzly, passes=None, threads=1):
    ...
    ft = ft[((ft['trip_distance'] != 0) &
            (ft['total_amount'] != 0) &
            (ft['pickup_longitude'] < -73) &
            (ft['pickup_longitude'] > -76) &
            (ft['pickup_latitude'] > 39) &
            (ft['pickup_latitude'] < 43) &
            (ft['dropoff_longitude'] < -73) &
            (ft['dropoff_longitude'] > -76) &
            (ft['dropoff_latitude'] > 39) &
            (ft['dropoff_latitude'] < 43) &
            (ft['passenger_count'] <= 3) &
            (ft['trip_distance'] != 0))]
...
if __name__ == "__main__":
    ...
    print "File={} Grizzly={} Threads={}".format(args.filename, True, args.threads)
    ...
    time = run(args.filename, False, passes=passes, threads=str(args.threads))
